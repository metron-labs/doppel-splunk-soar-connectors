...
class GetAllAlertsParams(Params):
   ...
   queue_state: str = Param(
       ...
       required=False,
       value_list=[
           "doppel_review", "needs_confirmation", "actioned",
           "taken_down", "monitoring", "archived"
       ],
   )
   product: str = Param(
       description="Product",
       required=False,
       value_list=[
           "domains", "social_media", "mobile_apps", "ecommerce",
           "crypto", "email", "paid_ads", "telco", "darkweb"
       ],
   )
...
class BaseAlertOutput(ActionOutput):
   ...
   doppel_link: str | None = OutputField(example_values=["https://app.doppel.com/alert/TST-123"])
...
def on_poll(
   ...
   if is_manual or not last_poll_time:
       start_ts = (now_utc - timedelta(days=asset.historical_polling_days)).strftime("%Y-%m-%dT%H:%M:%S")
   ...
   while True:
       ...
       for alert in alerts:
           ...
           for log in alert.get("audit_logs", []):
               audit_timestamp = (log.get("timestamp", "unknown")
                                .replace(":", "_").replace(".", "_").replace(" ", "_"))
               audit_type = (log.get("type", "unknown")
                           .replace(" ", "_").replace(":", "_").replace("/", "_"))
   ...
   if not is_manual:
       ...
       logger.info("Saved last_poll_time for next scheduled run.")
